...
class DataController {
    ...
    async getYoutubeData(req, res) {
        ...
        try {
            const page = parseInt(req.query.page) || 1;  // Get page number from query params (default is 1)
            const data = await dataService.getYoutubeData(page); // Fetch data using the service
    ...
    // the endpoint will be /search?q=wordtosearch
    async searchYoutubeData(req, res) {
        try {
            const searchString = req.query.q;
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 10; // Default limit = 10
            ...
            const data = await dataService.searchYoutubeData(searchString, page, limit);
    ...
    async YoutubegetMostViewed(req, res) {
            try {
                const page = parseInt(req.query.page) || 1;
                const data = await dataService.YoutubegetMostViewed(page);
